...
This is the ground-up route for a modder who has never used Beacon before. Follow it before choosing deeper crash, stats, or runtime API guides.

## What You Need

- A Hytale plugin or asset pack that you can package and install on a test server.
- Access to `https://beacon.modstats.io/portal`.
- A Discord or GitHub account for portal sign-in.
- For standalone dependency mode, the Beacon runtime installed alongside your package.
- For embedded mode, a plugin that can call Java runtime bootstrap code.

Content-only asset packs should normally use standalone dependency mode. They can ship a telemetry descriptor and use the installed Beacon runtime, but they cannot call custom runtime API methods unless they also include plugin code.

## Create The Portal Project

1. Open `https://beacon.modstats.io/portal`.
...
Use a short lowercase project ID such as `example-mod`, `beacon-cats`, or `my-asset-pack`. The display name can be friendlier, such as `Example Mod`.

## Choose Your Runtime Mode

Use [Standalone Dependency Mode](/mod/beacon/standalone-dependency-mode) when:

- you want the easiest setup
- your plugin or asset pack can use Beacon as an external dependency
- your package does not need to own telemetry startup code
- your asset pack has no Java code

Use [Embedded Mode](/mod/beacon/embedded-mode) when:
...
The project key is like an API key, so avoid posting it casually. It still cannot be kept truly secret once shipped in a Hytale package, so the Beacon portal has some preventative measures in the case of abuse, and you can rotate it from the portal if needed.

The portal project is the source of truth. The project key resolves to one portal project, and portal ingest rejects uploads when the descriptor's `projectId` does not match that portal project ID. If you omit `projectId`, Beacon infers it from your manifest `Name`, so choose the portal project ID to match the inferred value or add an explicit override.
...
If you want your project to show a custom icon in `/beacon consent`, add `ui.iconTexturePath` to the same descriptor:
...
3. Install Beacon too if you chose standalone dependency mode.
...
1. Return to `https://beacon.modstats.io/portal`.
2. Open the project.
...
- Use [Quick Crash Setup](/mod/beacon/quick-crash-setup) for automated crash reporting only.
- Use [Quick Stats Setup](/mod/beacon/quick-stats-setup) for anonymous public stats only.
- Use [Project Descriptor](/mod/beacon/project-descriptor) when you need every descriptor field.
- Use [Command Reference](/mod/beacon/command-reference) when a server operator needs the command list.
